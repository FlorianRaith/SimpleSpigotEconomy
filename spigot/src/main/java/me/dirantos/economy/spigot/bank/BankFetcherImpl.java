...
public final class BankFetcherImpl extends DataFetcherImpl<Bank, UUID> implements BankFetcher {
    ...
    private static final String CREATE_TABLE = "CREATE TABLE IF NOT EXISTS `banks` ( `id` INT UNSIGNED NOT NULL AUTO_INCREMENT , `uuid` VARCHAR(36) NOT NULL , `money` DECIMAL(15,2) NOT NULL , `account_numbers` VARCHAR(255) NOT NULL , PRIMARY KEY (`id`), UNIQUE (`uuid`))";
    private static final String INSERT_DATA = "INSERT INTO `banks` (`uuid`, `money`, `account_numbers`) VALUES (?,?,?) ON DUPLICATE KEY UPDATE `money` = VALUES(`money`), `account_numbers` = VALUES(`account_numbers`)";
    private static final String INSERT_MULTIPLE_DATA = "INSERT INTO `banks` (`uuid`, `money`, `account_numbers`) VALUES $values$ ON DUPLICATE KEY UPDATE `money` = VALUES(`money`), `account_numbers` = VALUES(`account_numbers`)";
    private static final String FETCH_DATA = "SELECT * FROM `banks` WHERE `uuid` = ?";
    private static final String FETCH_MULTIPLE_DATA = "SELECT * FROM `banks` WHERE `uuid` IN $values$";
    private static final String DELETE_DATA = "DELETE FROM `banks` WHERE `uuid` = ?";

    public BankFetcherImpl(MySQLConnectionPool mySQL, EconomyPlugin plugin, ModelCache cache) {
