...
import me.dirantos.economy.spigot.DataFetcherImpl;
import me.dirantos.economy.spigot.MySQLConnectionPool;
...
public final class TransactionFetcherImpl extends DataFetcherImpl<Transaction, Integer> implements TransactionFetcher {
    ...
    private static final String CREATE_TABLE = "CREATE TABLE IF NOT EXISTS `transactions` ( `id` INT UNSIGNED NOT NULL AUTO_INCREMENT , `recipient` INT UNSIGNED NOT NULL , `sender` INT UNSIGNED NULL , `amount` DECIMAL(15,2) NOT NULL , `type` VARCHAR(16) NOT NULL , `date` DATE NOT NULL , `interest_rate` DOUBLE NULL , PRIMARY KEY (`id`))";
    private static final String INSERT_DATA = "INSERT INTO `transactions` (`id`, `recipient`, `sender`, `amount`, `type`, `date`, `interest_rate`) VALUES (?,?,?,?,?,?,?) ON DUPLICATE KEY UPDATE `recipient` = VALUES(`recipient`), `sender` = VALUES(`sender`), `amount` = VALUES(`amount`), `type` = VALUES(`type`), `date` = VALUES(`date`), `interest_rate` = VALUES(`interest_rate`)";
    private static final String INSERT_MULTIPLE_DATA = "INSERT INTO `transactions` (`id`, `recipient`, `sender`, `amount`, `type`, `date`, `interest_rate`) VALUES $values$ ON DUPLICATE KEY UPDATE `recipient` = VALUES(`recipient`), `sender` = VALUES(`sender`), `amount` = VALUES(`amount`), `type` = VALUES(`type`), `date` = VALUES(`date`), `interest_rate` = VALUES(`interest_rate`)";
    private static final String FETCH_DATA = "SELECT * FROM `transactions` WHERE `id` = ?";
    private static final String FETCH_MULTIPLE_DATA = "SELECT * FROM `transactions` WHERE `id` IN $values$";
    private static final String DELETE_DATA = "DELETE FROM `transactions` WHERE `id` = ?";

    public TransactionFetcherImpl(MySQLConnectionPool mySQL, EconomyPlugin plugin, ModelCache cache) {
