...
public class Sidebar {
    ...
    private final ColorPool colorPool = new ColorPool();
    ...
    private void generateLines(int length) {
        ...
        for (int i = 0; i < length; i++) {
            Line line = getLine(i + beginIndex);
            if (line == null) {
    ...
    private void setText(String text, Line line) {
        if (text.length() > 30)
    ...
    private Line getNewLine(int index) {
        ...
        team.addEntry(entry);
        Line line = new Line(index, team, entry, color);
        update(line);
        return line;
    }
